export default function AssignProfessorModal({ open, onClose, teachers, selectedTeacherId, onSelect, onConfirm }) {
  if (!open) {
    return null;
  }
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-800 bg-slate-800/60 px-6 py-4">
          <div>
            ...
            <p className="text-xs text-slate-400">Matematicas Avanzadas I (CL-2024-001)</p>
          </div>
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          <p className="rounded-xl border border-sky-500/30 bg-sky-500/10 p-3 text-[11px] text-sky-100">
            Este cambio es visual para demo. No se guarda en backend en esta fase.
          </p>
        </div>

        <footer className="flex gap-3 border-t border-slate-800 bg-slate-800/40 px-6 py-4">
          <button
            className="flex-1 rounded-xl bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400"
            onClick={onConfirm}
            type="button"
          >
            Confirmar cambio
